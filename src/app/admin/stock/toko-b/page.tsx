...
export default function StockTokoBPage() {
  const db = useFirestore();
  const sourceStoreId = "TOKO_B";
  const sourceDisplayName = "IND CO";
  ...
  const handleMigrate = () => {
    ...
    // 2. UPDATE TARGET
    const targetStock = targetStoreId === "TOKO_A" ? (stockA || []) : (stockC || []);
    ...
    const targetDisplayName = targetStoreId === "TOKO_A" ? "NHS KWT" : "NHS GDM";
  ...
  const exportToPDF = () => {
    ...
    const tableData = flattenedVariants.map(v => [v.productName, v.brand, v.stock, `Rp ${v.price.toLocaleString('id-ID')}`]);
    (docPdf as any).autoTable({ head: [['Nama Barang', 'Merk', 'Stok', 'Harga Jual']], body: tableData, startY: 20 });
          ...
          <Table>
            ...
            <TableBody>{flattenedVariants.length > 0 ? flattenedVariants.map((item, idx) => (
                  <TableRow key={idx} className="hover:bg-muted/20">
                    <TableCell className="font-medium text-sm"><b>{item.productName}</b><br/><span className="text-[10px]">{item.category}</span></TableCell>
                    ...
                    <TableCell className="text-right font-bold">Rp {item.price.toLocaleString('id-ID')}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleSetZero(item.productId, item.id)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                    </TableCell>
                  </TableRow>)) : (<TableRow><TableCell colSpan={5} className="h-32 text-center text-muted-foreground">Kosong.</TableCell></TableRow>)}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
